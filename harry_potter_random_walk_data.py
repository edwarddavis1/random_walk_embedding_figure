...
for i in tqdm(range(num_epochs)):
    n2v_obj = Node2Vec_for_dyn_skip_gram(
        n_components=2, walklen=3, epochs=20, w2vparams={"window": 2}, keep_walks=True
    )
    ...
    if i == 0:
        ...
        np.savetxt(
            "hp_walks/walks_save{}.txt".format(i), np.array(n2v_obj.walks).astype(int)
        )
...
#     ya_t = np.loadtxt("hp_embedding_steps/wv_save" + str(i) + ".emb", skiprows=1)
#     ya_list.append(ya_t)
...
plot_embedding(ya, n, num_epochs, nodes)
