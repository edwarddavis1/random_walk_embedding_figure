...
class Node2Vec_for_dyn_skip_gram(BaseNodeEmbedder):
    def __init__(
        self,
        n_components=32,
        walklen=30,
        ...
        keep_walks=False,
        verbose=False,
        w2vparams={"window": 10, "negative": 5, "iter": 10, "batch_words": 128},
    ):
        """
        Parameters
        ----------
    ...
    def fit(self, G, update_file=None):
        """
        ...
        if update_file is None:
            ...
            # self.model = gensim.models.Word2Vec(
            # self.walks, size=self.n_components, **self.w2vparams)
            self.model = gensim.models.Word2Vec(
                size=self.n_components, **self.w2vparams
            )
            self.model.build_vocab(self.walks)
            total_examples = self.model.corpus_count
            previous_model = KeyedVectors.load_word2vec_format(
                update_file, binary=False
            )
            self.model.build_vocab([list(previous_model.vocab.keys())], update=True)
            self.model.intersect_word2vec_format(update_file, binary=False, lockf=1.0)
            self.model.train(
                self.walks, total_examples=total_examples, epochs=self.model.iter
            )
    ...
    def fit_transform(self, G):
        ...
        w = np.array(
            pd.DataFrame.from_records(
                pd.Series(np.arange(len(G.nodes()))).apply(self.predict).values
            )
        )
        return w

    def get_embedding(self, G):
        """Gets embedding after running self.fit"""
        w = np.array(
            pd.DataFrame.from_records(
                pd.Series(np.arange(G.shape[0])).apply(self.predict).values
            )
        )
        return w

    # def load_embedding(self, emb_file):
    #     """Initialise with past embedding"""
    #     self.model = gensim.models.Word2Vec(
    #         sentences=self.walks,
    #         size=self.n_components,
    #         **self.w2vparams)

    #     last_model = gensim.models.KeyedVectors.load_word2vec_format(
    #         emb_file, binary=False)
    #     n2v_obj.model.build_vocab([list(last_model.vocab.keys())], update=True)
    #     # lockf=1.0 allows updating of wv
    #     n2v_obj.model.intersect_word2vec_format(
    #         emb_file, binary=False, lockf=1.0)
    ...
    def load_vectors(self, out_file):
        ...
        """
        self.model = gensim.models.KeyedVectors.load_word2vec_format(out_file)


def dyn_skip_gram(As, d):
    ...
    ya_list = []
    for t in range(T):
        n2v_obj = Node2Vec_for_dyn_skip_gram(n_components=d, walklen=30, epochs=20)
        if t == 0:
            n2v_obj.fit(As[t])
            n2v_obj.save_vectors("walks/wv_save" + str(t) + ".emb")
            # n2v_obj.model.wv.save_word2vec_format("wv_save" + t + ".emb")
        else:
            n2v_obj.fit(As[t], update_file="walks/wv_save" + str(t - 1) + ".emb")
            n2v_obj.save_vectors("walks/wv_save" + str(t) + ".emb")
            # n2v_obj.model.wv.save_word2vec_format("wv_save" + t + ".emb")

        ya_t = np.array(
            pd.DataFrame.from_records(
                pd.Series(np.arange(As[0].shape[0])).apply(n2v_obj.predict).values
            )
        )
        ya_list.append(ya_t)
